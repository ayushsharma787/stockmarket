...
START_LONG   = "2005-01-01"   # Extended history (20 yrs is enough, avoids memory OOM)
END          = datetime.today().strftime("%Y-%m-%d")
XLS_PATH = "sp500_top10.xlsx"
# Pre-define as full list; gets narrowed to successfully-loaded tickers after data fetch
...
def fetch_yahoo():
    """Fetch daily data since START. Downloads all tickers in one batch call."""
    import yfinance as yf
    frames = {}
    try:
        # Batch download - much faster, fewer rate-limit hits
        raw = yf.download(
        ...
        # Fallback: individual downloads if batch fails
        for t in TICKERS:
            try:
...
def fetch_yahoo_long():
    """Extended history from START_LONG — loaded lazily, not at startup."""
    import yfinance as yf
    frames = {}
    try:
        raw = yf.download(
            TICKERS, start=START_LONG, end=END,
            progress=False, auto_adjust=True, group_by="ticker"
        )
        for t in TICKERS:
            try:
...
def _ensure_long_data():
    """
    Lazy-load extended history + MTF indicators — only called from pages
    that need long-term data, never at startup. Prevents OOM on Streamlit Cloud.
    """
    ...
    if not frames_long:
        with st.spinner("📅 Loading extended history (2005 → today)…"):
            try:
                ...
                frames_long = {}
        if not frames_long:
            frames_long = frames   # fallback: use daily data already loaded
    if not mtf_data:
        with st.spinner("📊 Building Weekly / Monthly / Quarterly indicators…"):
            try:
                mtf_data = build_mtf(frames_long)
            except Exception:
                mtf_data = {}
...
# Long-term data and MTF indicators are loaded LAZILY (only when those pages
# are selected) to avoid Streamlit Cloud memory/timeout crashes at startup.
frames_long: dict = {}
mtf_data:    dict = {}
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    _ensure_long_data()   # lazy: loads extended history + MTF indicators here
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    buf  = io.StringIO()
    _dl_ind = ind_data[dl_t] if dl_t in ind_data else ind_data[LOADED_TICKERS[0]]
    _dl_ind.to_csv(buf)
    st.download_button(
        label=f"⬇️ {dl_t} — Full Indicator CSV ({len(_dl_ind)} rows × "
              f"{len(_dl_ind.columns)} columns)",
        data=buf.getvalue(),
